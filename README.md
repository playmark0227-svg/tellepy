...
| モード | 説明 | 準備 |
|---|---|---|
| **Web自動探索** | **条件に合う企業の公開HPをWebから自動で探してリスト化**（既定）。電話番号も取れる | なし（任意で検索APIキー） |
| **gBizINFO 自動巡回bot** | gBizINFOを巡回し、資本金・従業員数で厳密に絞る | APIトークン |
| **ローカルCSV** | **PC内のCSVだけで検索**。`data/` にCSVを置くだけ | CSVを1つ置く |
| **デモ** | サンプルデータで画面確認 | なし |
