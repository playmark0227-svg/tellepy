...
| **自前データ＋無料エンリッチ** | **国税庁の全件データ（無料）**で母集団を作り、無料Web検索でHP・電話番号を補完 | 不要（自動取得・自動更新） | ゼロ |
...
**① 無料の企業母集団（自動取得・自動更新） → ② 条件で絞り込み → ③ 無料Web検索でHP・電話番号を補完**
まで一気通貫でやります。

1. **母集団は自動で最新に保たれます**: `python main.py` を起動すると、国税庁「法人番号公表サイト」の
   全件データ（月次更新・無料・トークン不要・全国約500万法人）を **起動時＋1日1回自動チェック**し、
   新しい月次データが公開されていれば自動でダウンロード→変換して `data/` に反映します（[`nta_updater.py`](nta_updater.py)）。

   - 対象の都道府県は `NTA_PREFECTURES`（既定: 東京都,神奈川県,千葉県,埼玉県）で変更できます。
     `一都三県` `関西` `東海` `九州` などのまとめ言葉や、`東京` のような略称も使えます
   - 自動更新を止めたい場合は `NTA_AUTO_UPDATE=0`。管理画面の「国税庁データを今すぐ更新」で手動実行も可能
   - 取得状況（何年何月版・何社）は管理画面「リスト作成」に表示されます
   - 国税庁データは **社名＋所在地＋郵便番号のみ**（電話番号・業種・資本金・従業員数は含まない）

   手動でダウンロード済みのファイルがある場合は、**生の全件CSV（ヘッダ無し）をそのまま `data/` に
   置くだけでもOK**（形式を自動判別）。従来どおり [`corp_importer.py`](corp_importer.py) での事前変換にも対応:
...
##### AIフォールバック（迷った会社だけHaikuで確認・ハイブリッド）

無料の正規表現・名寄せでは「どのサイトが公式か」「電話番号がどれか」を確定できない会社が一定数出ます。
そこだけを **Claude Haiku 4.5（現行最安）** に確認させると、抽出精度を大きく底上げできます。

- 詳細オプションの **「迷った会社だけAIで確認…」** を ON にし、`ANTHROPIC_API_KEY` を設定すると有効化されます
- **キー未設定なら自動で無効**（＝完全に従量課金ゼロのまま）。ONでも **確定できなかった会社にしかAIを呼ばない**
- 候補は最大3件・各本文1500字までに切ってから渡すので、1回あたりのコストはごくわずか
- 実際に叩いたAI回数は結果画面の **「AI確認(有料)」** と進捗表示に出ます（コストの実数が見える）
...
ブラウザ版の既定の探し方です。**手元に企業名簿を1つも持っていなくても**、
依頼文と業種・エリアを指定するだけでリストができます。

- Anthropic の Web検索ツールは Anthropic 側のサーバーで走るため、
  ブラウザから直接呼んでも CORS に当たりません。これが成立の根拠です
- 公開されているホームページから **社名・所在地・電話番号・公式サイトURL** を拾うので、
  出来上がった時点でそのまま架電できます（名簿から作る場合は電話番号が別途必要）
- でっち上げ対策として、**出どころのURLを示せない会社は採用しません**。
  新しい会社が出てこなくなったら、数合わせをせずそこで止めます
- 規模（従業員数・資本金）は「AIへの希望」として伝えますが、裏取りはできないので
  画面にもその旨を表示します
- 費用の目安: **1,000社で概算 ¥800**（Haiku 4.5・Web検索料込み）。
  上限トークンに達した時点で自動的に止まります

無料で同じことをしたい場合は、ローカル版（`python main.py`）の「Web自動探索」を
使ってください。無料の検索エンジンで巡回するため 0円で済みます。
...
| ケース | AIを呼ぶ件数 | 概算 |
|---|---|---|
| 想定（迷った2〜4割にだけAI） | 200〜400件 | **¥300〜¥800** |
| 多め（半分にAI／複数候補を確認） | 500〜1,500回 | **¥500〜¥1,500** |
| 最悪（全件をAIに通す） | 1,000件 | **〜¥1,500程度** |

> 月額基本料（税込¥33,000）に対して、1リストの外部コストは数百円〜せいぜい¥1,500程度に収まります。
> 「網羅性は自前データ、抽出の賢さはAI、コストはほぼゼロ」のいいとこ取りです。
